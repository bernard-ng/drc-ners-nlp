...
from drc_names_classifier.web.components import (
    render_dataset,
...
def render_app() -> None:
    ...
    st.set_page_config(
        page_title="DRC Names Classifier",
        layout="wide",
        initial_sidebar_state="expanded",
    )
    config = ExperimentSettings()

    st.title("DRC Names Classifier")
    ...
    with st.sidebar:
        ...
        st.divider()
        st.caption(
            "Paths and defaults come from drc_names_classifier.config. This interface never writes a "
            "derived dataset."
        )
